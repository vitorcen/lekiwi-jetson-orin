...
- **会话历史会自我强化复读**：噪音起轮 → 模型瞎编一句 → 进历史 → 下轮复读 → 再进历史。
  ...
  **但判据不能是字符串相等** —— 见下节，这条第一版就是栽在这里。
- **只出声不出字**：GUI 对话气泡渲染的是 `assistant_delta`，不是 `tts`。omni 无 token 流，
  整句作为一个 delta 发出。

## 模型复读的是**句式**，不是字符串 —— 去重判据必须是相似度

2026-07-26 现场：omni 对每一句都答「我正在听你说，关于**X**的事。」X 随用户话题变
（学校/牛/下雨/东西），像是没听懂。**不是没听懂**——同一段音频、同一提示词、同一模型，
换个全新 `conversation_id` 八轮全是正常回答且能跨轮指代；线上那个会话就吐模板。
**根因是历史锁死**，而上一节那条「重复就两份都删」的去重**根本没触发**，因为它比的是
`==`：四句模板两两只差一个名词，字符串各不相同，全部照收。四轮之后上下文里
100% 是这一句的四个副本，锁死不可逆。

修法两条，缺一不可：
1. **判据换成相似度**（`difflib.SequenceMatcher.ratio() >= 0.6`）。阈值不是拍的：
   实测模板家族两两 **0.86–0.89**，健康会话八句真回答两两 ≤ **0.37**、对模板 ≤ **0.26**，
   0.6 落在 2 倍余量的空隙里。
2. **清整条历史**，不是只清最后一对。只清一对的话四层锁要四轮才排干，
   而这四轮用户每轮都要听一遍那句废话；全清是一轮恢复。
   实测锁死路径现在震荡在 0↔1 个 exchange，永不累积。

**How to apply:** 看到「答非所问但话题词对得上」，先换个 `conversation_id` 复现一次
——分得清「模型不行」和「上下文脏了」，这两个方向的修法完全相反。
别再写 `==` 当复读判据。见 [[measurement-validity]]。
