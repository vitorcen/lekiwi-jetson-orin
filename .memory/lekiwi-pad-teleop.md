...
**六关节限幅（2026-07-30 真机修复）**：手柄遥控使用从臂校准文件中 1–6 全部关节的
实际 `range_min/range_max`。腕旋转 ID 5 也受校准范围约束，不能跳过它、不能以启动位置
± 固定值代替，更不能强制保存为 `0..4095`；这样会让主从两边的腕旋转与校准记录严重
不一致。校准时六个关节都要手动覆盖各自允许的两个安全端点。
