...
故 offhook 写不是根因。**终审(2026-07-21):这台 MCP01 是坏件——`lsusb` 显示它是
`Bus001 Device 101`(USB 设备号每次重连+1,101=已闪断约100次),且会把 voice-daemon
拖死在 systemd `deactivating`(采集卡住无法干净停),重启也白搭。软件不该被坏 USB 拖挂
(该给 arecord 加有界超时+退避,是真 bug 待修),但根子是硬件:换掉 MCP01、插 USB 麦
(daemon 自动认 "USB Audio" 卡)。远场会议音响+不稳 USB 对机器人近场指令是双重错配。**
缓解三件套(均已落地,默认零行为变化):
