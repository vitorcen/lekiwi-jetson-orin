...
description: Waveshare ESP32-S3-AUDIO-Board 已改 UAC；共享 I²S 必须 2×32-bit standard，ADC 37.5dB；半米 TTS→Qwen3 ASR 逐字通过
metadata:
  type: project
---
...
语音，正常近场峰值应明显越过 −25 dBFS。Jetson 使用 `plughw:CARD=Audio` 做
24→16 kHz 转换。恢复 AI 固件前先校验备份哈希。与 [[voice-frontend-s2]]、
