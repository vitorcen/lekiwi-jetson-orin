...
**网关运维(2026-07-19 教训)**:龙虾网关 = systemd 用户服务 `hermes-gateway-robot`,
重启用 `systemctl --user restart hermes-gateway-robot`。**裸敲 `hermes gateway restart`
(不带 `--profile robot`)起的是默认 profile 的前台网关**——会连上 Lark、阻塞终端、
和 robot 网关并存,看着像"重启失败"。另:改了 MCP 服务器源码(如 drive/vlm)必须
重启网关才生效——MCP 是网关拉起的子进程。
