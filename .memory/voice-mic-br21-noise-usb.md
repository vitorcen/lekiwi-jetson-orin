...
60Hz/100Hz 电源线频；风扇 pwm=100/255 在转。150Hz 高通把底噪压到 −47.2（**空录时
白赚 8 dB**，120Hz 也有 7.3 dB），而 150Hz 以下对识别几乎无信息（电话带宽 300–3400Hz）。

> **⚠ 别去做那个高通,实测无效（2026-07-28,n=30 复现过）**：信噪比净增只有
> **+4.1 dB**（语音自己也有低频,一起被砍）。而且实测**对 VAD 完全没有影响**:
> 新录音各播 3 遍,音量69 有无高通都是 30 段/25 句全对,音量50 都是 0 段。
> (中间一度在单份录音上量出 1→9 段的大胜,换录音复现不出来 —— n=1 抽签。)
> 详见 [[vad-asr-tuning-results]] 末节。
