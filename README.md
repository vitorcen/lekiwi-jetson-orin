...
汇总手柄、键盘、主臂三路操作。连接行的「切断电机输出」是安全总开关——切断轮子和
机械臂全部舵机扭矩并闩锁在板端,
...
首选 GUI ZeroMQ 页「主臂遥操作」面板里的**主臂校准 / 从臂校准**。两者都按
**摆中间位 → 记录 → 每关节手动拉到最大最小 → 完成保存**执行，取消会恢复旧 EEPROM
参数，活动范围不足的关节会拒绝保存。从臂结果与 LeRobot 共用
`~/.cache/huggingface/lerobot/calibration/robots/lekiwi/orin_kiwi.json`。

无 GUI 时仍可用 lerobot CLI。它需要交互终端，在板子上跑：
